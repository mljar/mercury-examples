...
## Import
It's time to **install Mercury** (if you haven't done this already). 
```
pip install mercury
or
...
Now that we have assigned a category to each job listing, we can further clean the data by removing job listings that belong to the "unknown" category. We will also remove any duplicates in the data.
```
...
The second line of code **remote_jobs_percent** calculates the percentage of rows in the df_lvl that have the value 'True' in the 'work_from_home' column, **rounds the result to one decimal place**, and assigns the value to the variable remote_jobs_percent.

![markdown-txt][2]

## Word Cloud
To add some interesting to our presentation we can **add Word Cloud.** This is a **collection of all the sub-counted words,** the size of the word depends on the number of times the word is repeated in the text.
